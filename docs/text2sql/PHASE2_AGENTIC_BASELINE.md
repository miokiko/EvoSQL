...
Text2SQL Lead（路由、委派）
  └─ Evidence Orchestration
       ├─ 正向 LLM Linking + 关键词匹配
       ├─ Vanna DDL/文档/确认 SQL → 不执行的草稿 SQL
       ├─ 草稿 AST 反向提取 + Snapshot 扩展 + 条件补充检索
       ├─ GroundingPack/v1 + SchemaLinkPack/v3 candidates
...
Harness 召回并复验 Vanna Question-SQL ──→ VerifiedExamplePack/v1（最多 3 条）
SQL Generation（ApprovedQueryPlan 为唯一语义约束）──→ 最多 4 个只读 SQLCandidate
...
Worker 彼此不通信。Query Planning 对物理 Schema 保持 blind：只看到问题和业务 Markdown 提供的业务术语，不接收 DDL、物理列、实库值、关系或 SQL 示例。Schema Grounding 与 Query Planning 之外的节点均按固定顺序串行执行。数据库始终以 `mode=ro&immutable=1 + PRAGMA query_only=ON` 打开。

FOLLOW_UP_QUERY 必须由 Lead 明确引用当前 session 内存在的父 QueryRun；未给出、跨会话、失败、版本漂移或计划指纹不一致的 parent 会直接 fail closed，不会自动回退到“最近一轮”，也不能继续使用 Lead 的 standalone rewrite 生成 SQL。服务端按 session 重新读取父结果，只有 `status=success` 且最终 Harness Gate 已接受的结构化 QuerySpec / SchemaPlan 才能提供继承值或显式 Join。Lead 写出的 standalone question 只帮助 Worker 理解追问，不会扩大任何事实来源；其中新增的物理 Schema 或 SQL 会在进入 Planning 前被撤回。
...
输出包含最终 SQL、结果、Database Snapshot / Vanna corpus / Memory / Policy 四个逻辑版本固定值、Gate、协作轨迹和模型/工具调用统计。业务文档摘要已包含在 corpus fingerprint 中；wire 兼容字段 `wiki_index_version` 保存同一个 Vanna corpus version，不是独立 Wiki Pin。没有配置模型时不会降级成单 Agent 或规则生成器，而是直接拒绝启动。Web 服务或直接调用 Engine 时若注入 checkpoint store，并传入稳定 `task_id`，11 个固定 Runtime Node 会逐节点持久化；CLI 与评测脚本的当前恢复边界见 `CHECKPOINT_RUNBOOK.md`。
...
- logical value 必须具有可信来源：保守匹配本轮原始用户问题中的表面形式，或继承自同一 session 下、最终 Gate 已接受的父 QueryRun 的类型化 filter；Lead 生成的 standalone rewrite 本身永远不是来源；
- 非同值的 logical-to-physical 映射必须来自 `schema-grounding` 独占的业务文档 value alias；LIKE 仅额外允许 Harness 确定性添加首尾 `%`；
- `eq` / `in` 的 physical value 必须在固定只读数据库中真实存在；范围边界与 LIKE 模式不要求恰好命中某行，但必须与固定列类型兼容；
- SchemaBinding 必须属于本轮固定 Schema Snapshot；Join 必须来自独立 Join Catalog 的可用条目，模型填写的任意 evidence id 不构成授权；
...
- SQL Generation 只能以由 Lead 做语义决策、Harness 铸造的不可变 ApprovedQueryPlan 作为语义约束；可选 Question-SQL 仅在计划批准后由 Harness 从 Vanna 检索，必须经当前 Snapshot、SQL 安全和计划表列范围复验，并且最多 3 条；
...
python -m unittest \
  ...
  tests.test_text2sql_vanna_retriever \
